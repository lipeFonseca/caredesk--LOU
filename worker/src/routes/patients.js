...
import { calcProtocolUrgency, parseProtocolDays } from '../utils/protocols.js'
...
patients.get('/', async (c) => {
  ...
  // Fallback: protocolo padrão (is_default=1) ou global settings
  const defaultProto = await c.env.DB.prepare(
    "SELECT days FROM contact_protocols WHERE is_default = 1 LIMIT 1"
  ).first()
  const globalSetting = await c.env.DB.prepare(
    "SELECT value FROM app_settings WHERE key = 'contact_protocol_days'"
  ).first()
  const fallbackDays = defaultProto?.days
    ? JSON.parse(defaultProto.days)
    : globalSetting?.value
      ? JSON.parse(globalSetting.value)
      : [-2, 0, 2, 5, 15, 30, 60, 90, 120, 180]

  const today = new Date().toISOString().split('T')[0]
  const enriched = results.map(p => {
    const days = parseProtocolDays(p._proto_days, p.protocol_days, fallbackDays)
    const { _proto_days, ...rest } = p
    return { ...rest, followup_urgency: calcProtocolUrgency(rest, days, today) }
  })
...
patients.get('/:id', async (c) => {
  ...
  // Parsear protocol_days_json se vier do JOIN
  const patientOut = { ...patient }
  if (patientOut.protocol_days_json) {
    patientOut.protocol_days_parsed = parseProtocolDays(patientOut.protocol_days_json, patientOut.protocol_days)
  }
  if (!patientOut.protocol_days_parsed?.length) {
    patientOut.protocol_days_parsed = parseProtocolDays(patientOut.protocol_days)
  }

  return c.json({ ...patientOut, followup_logs: logs })
})
...
patients.post('/', async (c) => {
  ...
  // Resolve protocol_id: use provided, or find default
  let resolvedProtocolId = protocol_id || null
  if (!resolvedProtocolId) {
    const def = await c.env.DB.prepare('SELECT id FROM contact_protocols WHERE is_default = 1 LIMIT 1').first()
    resolvedProtocolId = def?.id || null
  }
...
patients.patch('/:id', async (c) => {
  ...
  const values = fields.map(f => (nullable.includes(f) && body[f] === '' ? null : body[f]))
